...
public:
    explicit ArrangeableWT(const ObjectID                  &oid,
        ...
        , m_xl_bb{xl_bb}
        , m_plate_idx{wti.plate_index()}
    {}

    // Rotation is disabled for wipe tower in arrangement
    void transform(const Vec2d &transl, double /*rot*/) override
    {
        GLCanvas3D::WipeTowerInfo::apply_wipe_tower(m_plate_idx, m_orig_tr + transl, m_orig_rot);
    }
...
arr2::SceneBuilder build_scene(Plater &plater, ArrangeSelectionMode mode)
{
    ...
    builder.set_arrange_settings(init_arrange_params(&plater));
...
FillBedJob2::FillBedJob2(std::shared_ptr<ProgressIndicator> pri, Plater *plater, arr2::Scene &&scene, const Callbacks &cbs)
    : Base(std::move(pri), plater, std::move(scene), _u8L("Filling bed"), cbs)
{}

ArrangeJob2::ArrangeJob2(std::shared_ptr<ProgressIndicator> pri, Plater *plater, arr2::Scene &&scene, const Callbacks &cbs)
    : Base(std::move(pri), plater, std::move(scene), _u8L("Arranging"), cbs)
{}
