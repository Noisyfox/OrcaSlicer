    ...
    public:
        ...
        void arrange2(arr2::Scene &&scene, const ArrangeJob2::Callbacks &cbs)
        {
            ...
            if (m_arrange2_id == JOB_NONE) {
                m_arrange2_id = add_job(std::make_unique<ArrangeJob2>(m_pri, m->q, std::move(scene), cbs));
            } else {
                replace_job(m_arrange2_id, std::make_unique<ArrangeJob2>(m_pri, m->q, std::move(scene), cbs));
            }
            start(m_arrange2_id);
        }
        ...
        void fill_bed(arr2::Scene &&scene, const FillBedJob2::Callbacks &cbs)
        {
            ...
            if (m_fill_bed_id == JOB_NONE) {
                m_fill_bed_id = add_job(std::make_unique<FillBedJob2>(m_pri, m->q, std::move(scene), cbs));
            } else {
                replace_job(m_fill_bed_id, std::make_unique<FillBedJob2>(m_pri, m->q, std::move(scene), cbs));
            }
            start(m_fill_bed_id);
        }
