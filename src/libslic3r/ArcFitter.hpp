...
public:
    ...
    static void do_arc_fitting_and_simplify(Points& points, std::vector<PathFittingData>& result, double arc_tolerance, double dp_tolerance);
    static void do_arc_fitting_and_simplify(Points& points, std::vector<PathFittingData>& result, double tolerance)
    {
        do_arc_fitting_and_simplify(points, result, tolerance, tolerance);
    }
};
