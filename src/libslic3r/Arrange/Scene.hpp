...
protected:
    ...
    AnyPtr<const arrangement::ArrangeParams> m_settings;
...
public:
    ...
    Subclass &&set_arrange_settings(AnyPtr<const arrangement::ArrangeParams> settings)
    {
        m_settings = std::move(settings);
        return std::move(static_cast<Subclass&>(*this));
    }

    Subclass &&set_arrange_settings(const arrangement::ArrangeParams &settings)
    {
        m_settings = std::make_unique<arrangement::ArrangeParams>(settings);
        return std::move(static_cast<Subclass&>(*this));
    }
...
public:
    ...
    const arrangement::ArrangeParams &settings() const noexcept { return *m_settings; }
...
void SceneBuilderBase<Subclass>::build_scene(Scene &sc) &&
{
    ...
    if (!m_settings)
        m_settings = std::make_unique<arrangement::ArrangeParams>();

    coord_t inset = std::max(m_settings->min_bed_distance,
                             m_skirt_offs + m_brims_offs);

    coord_t md = m_settings->min_obj_distance;
    md = md / 2 - inset;
