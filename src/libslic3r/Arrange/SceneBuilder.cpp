...
coord_t get_skirt_inset(const Print &fffprint)
{
    ...
    if (fffprint.has_skirt()) {
        ...
        skirt_inset = fffprint.config().skirt_loops.value * skirtflow
...
coord_t brim_offset(const PrintObject &po)
{
    ...
    const float    brim_separation = po.config().brim_object_gap.getFloat();
...
{
    BoundingBoxf3 bb;
    const Transform3d inst_matrix = mi.get_transformation().get_matrix(dont_translate);
