...
void fill_rotations(const Range<It>           &items,
                    const Bed                 &bed,
                    const arrangement::ArrangeParams &settings)
{
...
class DefaultArranger: public Arranger<ArrItem> {
    arrangement::ArrangeParams m_settings;
        ...
        ArrangerCtl<ArrItem> &ctl)
    {
        ...
        switch (m_settings.arrange_strategy) {
        default:
            [[fallthrough]];
        ...
        if (!with_wipe_tower &&
            m_settings.arrange_strategy == ArrangeSettingsView::asAuto &&
...
public:
    explicit DefaultArranger(const arrangement::ArrangeParams &settings):m_settings(settings)
    {
    }
