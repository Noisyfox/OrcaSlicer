...
void ArcFitter::do_arc_fitting_and_simplify(Points &points, std::vector<PathFittingData> &result, double arc_tolerance, double dp_tolerance)
{
    //BBS: 1 do arc fit first
    if (abs(arc_tolerance) > SCALED_EPSILON)
        ArcFitter::do_arc_fitting(points, result, arc_tolerance);
    else
    ...
    if (result.size() == 1 && result[0].path_type == EMovePathType::Linear_move) {
        ...
        points = MultiPoint::_douglas_peucker(points, dp_tolerance);
        ...
        for (size_t i = 0; i < result.size(); i++)
        {
            ...
            straight_or_arc_part = MultiPoint::_douglas_peucker(straight_or_arc_part, dp_tolerance);
